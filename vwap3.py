...
# MAIN ALGO LOOP
def algo_loop(trading_day, order_side, original_order_quantity, vwap_coefficients, max_behind, schedule_coef, tick_window, tick_coef, limit_price = None, limit_quantity = np.inf):
    ...
    for index, row in trading_day.iterrows():
        ...
        if order_side == 'b' :  
            if (fair_value >= ask_price and quantity_behind > round_lot) or (minutes_from_open==last_minute_from_open and quantity_behind > 0):
                ...
                if trades.loc[index].trade_type=='p' or ask_price > limit_price:
                    continue
                ...
                if minutes_from_open == last_minute_from_open:
                    ...
                    new_order_quantity = min(calc_order_quantity(quantity_behind, round_lot),limit_quantity)
            ...
            if (fair_value <= bid_price and quantity_behind > round_lot) or (minutes_from_open==last_minute_from_open and quantity_behind > 0):
                ...
                if trades.loc[index].trade_type=='p' or bid_price < limit_price:
                    continue
